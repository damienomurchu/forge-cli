...
// This test requires Unix pseudo-terminal semantics.
package promptprobe
...
func TestCtrlCRestoresTerminalState(t *testing.T) {
	...
	cmd := exec.Command(os.Args[0], "-test.run=TestPromptHelperProcess")
	cmd.Env = append(os.Environ(), "FORGE_PROMPT_HELPER=1")
	...
	for !bytes.Contains(output, []byte("Kind")) {
		select {
		case read := <-reads:
...
func TestPromptHelperProcess(t *testing.T) {
	...
	prompt := Prompt{Input: os.Stdin, Output: os.Stderr}
